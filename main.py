"""Aggregates New Jersey-located listings from every taildraggers.com
...
ones whose location ends in ", NJ".
...
Minnesota/Mississippi/Missouri/Montana/Nebraska/Nevada/New Hampshire repos'
approach, with the state filter swapped to "NJ".
"""
...
PAGE_TITLE = "Taildragger Ads in New Jersey"
STATE_ABBR = "NJ"
...
_NEW_JERSEY_LOCATION_RE = re.compile(r",\s*NJ\s*$", re.IGNORECASE)


def _is_new_jersey(location: str) -> bool:
    return bool(_NEW_JERSEY_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        new_jersey_rows = [row for row in rows if _is_new_jersey(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(new_jersey_rows)} in New Jersey")
        all_listings.extend(new_jersey_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No New Jersey listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique New Jersey listings: {len(listings)}")
