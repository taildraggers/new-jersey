...
Daily aggregator of New Jersey-located taildragger classified
...
repos' approach, with the state filter swapped to "NJ".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NJ` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in New Jersey"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/new-jersey/`
...
```html
<iframe
  src="https://taildraggers.github.io/new-jersey/"
  title="Taildragger Ads in New Jersey"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NEW_JERSEY_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
